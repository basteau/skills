...
WCAG target-size requirements, larger usability targets and pseudo-element expansion belong to [better-accessibility](../better-accessibility.md); these clearances are in addition, so expanded hit areas never overlap.
...
- **Truncation cues.** Clamped text shows an ellipsis and a way to expand. Truncation mechanics are [better-typography](../better-typography.md)'s.
