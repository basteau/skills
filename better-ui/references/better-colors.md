...
Contrast requirements belong to [better-accessibility](better-accessibility.md). Surfaces, shadows and icon color belong to [better-polish](better-polish.md).
...
For a new system, `oklch()` is the best default, because its numbers behave the way the ramp rules below describe. Everywhere else, a color library produces the same ramp in the project's own notation ([color-formats.md](better-colors/color-formats.md)).
...
That seam is what makes theming possible. Without it, dark mode means auditing every usage to work out which meant "the accent" and which just wanted blue ([token-naming.md](better-colors/token-naming.md)).
...
Both ends stop short of pure black and white, which cannot carry hue at all. Use a color library rather than eyeballing it ([palette-generation.md](better-colors/palette-generation.md)).

## One color, one meaning

Use a color for one purpose across the whole interface, treating anything within `15°` of hue as the same color. If the accent means interactive, that hue on static text tells users to click something that is not clickable, and an interactive element rendered neutral misleads just as badly. Color is never the only carrier of meaning, which [better-accessibility](better-accessibility.md) owns.
...
See [color-usage.md](better-colors/color-usage.md).
